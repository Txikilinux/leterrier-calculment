...
void exercice::adapte(QPixmap cheminImage)
{
    ...
    *m_imgFond = imgFond2;
    QBrush* fond = new QBrush(imgFond2);
            ...
            m_ui->vue->setGeometry(this->geometry().x()+ 120, this->geometry().y()+bordure, imgFond2.width(), imgFond2.height());

            m_scene->setSceneRect(0, 0, imgFond2.width(), imgFond2.height());

                                                                //            this->setGeometry(10,20, imgFond2.width()*1.223,imgFond2.height()*1.05);
            this->setGeometry(ecran.width()/2 - this->width()/2,ecran.height()/2 - this->height()/2 , imgFond2.width()+120,imgFond2.height()+50); //mais ça sert à rien ça, vu la ligne dessous...
